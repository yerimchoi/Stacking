...
class DiversityMeasure:
    def __init__(self, yhat1, yhat2, yhat3, acc, auc):
        self.yhat1 = yhat1
        self.yhat2 = yhat2
        self.yhat3 = yhat3
        self.acc = acc
    ...
    def transform_yhat(self):
        list = [self.yhat1, self.yhat2, self.yhat3]
        return list
...
if __name__ == "__main__":
    ...
    model_list = []
    models = list(combinations(model_nlsdk, 3))
    ...
    for model in model_list:
        ...
        for file in file_list:
            ...
            for j in range(len(meta_csv)):   # meta performance 한 행에 대하여,
                yhat1 = yhat_csv[3*j, :]
                yhat2 = yhat_csv[3*j+1, :]
                yhat3 = yhat_csv[3*j+2, :]

                row      = meta_csv[j]
                test_acc = row[5]
                test_auc = row[6]

                diversity = DiversityMeasure(yhat1, yhat2, yhat3, test_acc, test_auc)
                diversity_acc_list = diversity.gather_measure_acc()  # [mean_q, mean_lo, mean_dis, mean_df, mean_dc, acc, auc]
